...
2. **可配置性**:每个具体实现都用"dispatch policy" tag(空 struct)标记,builder(第 9 章)用 tag 路由到对应 partial specialization——用户写 `Auto` 而不是写具体类名。
...
教程共 10 章 + 4 附录。按"对 CUTLASS 3.x 抽象的覆盖深度"分两档:
...
必修 (Core Path)        ── 9 章构成主干,读完即能读懂 examples/48 + 99% 的 mainloop
  ...
  Ch4  深入 CollectiveMainloop
  Ch5  深入 CollectiveEpilogue
  Ch6  Epilogue Visitor Tree(EVT)
  Ch7  Kernel orchestrator + TileScheduler(调度器族对比)
  Ch8  DispatchPolicy(tag-inheritance 模式)
  Ch9  CollectiveBuilder(8 个 partial spec 路由)
...
```text
              ...
              ┌───────────────┼───────────────┐
              ▼               ▼               ▼
        Ch2 (CuTe)      Ch4 (mainloop)  Ch5 (epilogue)
              │               │               │
              ▼               │               ▼
        Ch3 (smem pipeline)   │          Ch6 (EVT)
              │               │               │
              └───────────────┴───────────────┘
                              │
                              ▼
...
- **第二次读**(按需):补 Ch10(Blackwell,如果你用 sm100 / sm103)。

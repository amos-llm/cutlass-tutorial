...
- [第 9 章:CollectiveBuilder——把「形状 + 类型」压成具体实现(含全配置空间地图)](docs/09-collective-builder.md)

### 进阶(Advanced)——按需读

- [第 10 章:Blackwell 桥接——同样的 5 层架构,换了一组原子](docs/10-blackwell.md) (用 sm100 / sm103 必读)
- [第 11 章:Blackwell 物理基础——TMEM + UMMA + CLC](docs/11-blackwell-physics.md) (深入物理细节)
...
3. **第二次读**(按需):补 [第 10 章](docs/10-blackwell.md)(如果你用 sm100/103)。**深入物理细节**再看 [第 11 章](docs/11-blackwell-physics.md)。
4. 附录是速查表,不需要从前往后读。

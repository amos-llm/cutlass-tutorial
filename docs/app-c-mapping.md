...
|`__shared__ float smem_A[TileM][TileK];` + swizzle|`SmemLayoutAtomA = composition(Swizzle<B,M,S>, Layout<Shape<_TileM, _TileK>, Stride<_TileK, _1>>{})`|Ch9|
...
|`wgmma.mma_async.sync.aligned.m64n...k16...`(单条)|`cute::gemm(TiledMma, A_frag, B_frag, acc)` (内部 dispatch 到该指令)|Ch2.5|
|`is_producer = warpIdx < 4` 分支|`WarpGroupRole { Producer, Consumer }` + `kernel/.../operator()`|Ch7.3|
|producer/consumer 屏障数组(`bar[N]`)|`cutlass::PipelineTmaAsync<N>`|Ch4.2 + Ch7.4|
...
|cluster 同步 `cluster_arrive + cluster_wait`|`cute::cluster_arrive + cute::cluster_wait`|Ch7.3|
|`grid = ceil_div(M, BlockM) * ceil_div(N, BlockN);`|`PersistentTileSchedulerSm90Params::num_blocks_in_grid`|Ch7.5|
|`blockIdx.x` 决定本 CTA 处理哪个 tile|`PersistentTileSchedulerSm90::fetch_next_work(...)`|Ch7.5|
|swizzle 步进(swizzle_pattern[blockIdx.x])|`max_swizzle_size = ...` 在 `arguments.scheduler` 中|Ch2.4 + Ch7.5|
...
|Stream-K(把 K 维继续切,partial sum)|`TileSchedulerType = StreamKScheduler`|Ch7.7|
